...
```{r}
  x = seq(-100, 100, 0.1)
  y = x^2
  matplot(x, y, type="o", col="black", lwd = 3,xlim = c(-4,4), ylim = c(-1.5, 1.5))
  abline(h=0)
  abline(v=0)
  colors=c("#e6194B", "#3cb44b", "#4363d8", "orange", "brown");
  
  for (i in 1:ncol(W)) {
    #randomnum = sample(1:length(colors), 1)
    #color = colors[randomnum]
    lines(z, W[,i], type = "o", col=colors[i], lwd = 3,ylim = c(-length(z), length(z)))
  }
